use std::{ fs::{ self, File }, path::Path, io::{ BufRead, BufReader } };

use axum::{ response::{ Html, IntoResponse }, extract::Path as extractPath };
...
pub async fn blog_page(extractPath(file_name): extractPath<String>) -> impl IntoResponse {
    ...
    let markdown_input = fs
        ::read_to_string(markdown_file_path)
